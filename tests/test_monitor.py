"""Tests for agentra.monitor — tracer, drift, daemon."""
import pytest
import time
...
class TestTracer:
    def test_trace_context_manager(self):
        from agentra.monitor.tracer import trace
    ...
    def test_trace_ids_propagated(self):
        from agentra.monitor.tracer import trace, span, _current_trace_id
...
class TestPricingAndProviders:
    ...
    def test_get_cheaper_alternative(self):
        from agentra.pricing import get_cheaper_alternative
        alt = get_cheaper_alternative("gpt-4o")
        assert alt is not None
        assert alt == "gpt-4o-mini"

    def test_list_models(self):
        from agentra.pricing import list_models
        models = list_models()
