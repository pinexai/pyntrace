"""Tests for agentra/guard/pii_mask.py"""
import os
import pytest
...
def clear_mask_env(monkeypatch):
    monkeypatch.delenv("AGENTRA_MASK_PII", raising=False)
...
def test_mask_email(monkeypatch):
    monkeypatch.setenv("AGENTRA_MASK_PII", "1")
    from agentra.guard.pii_mask import mask_pii
...
def test_sanitize_for_log_always_active():
    from agentra.guard.pii_mask import sanitize_for_log
