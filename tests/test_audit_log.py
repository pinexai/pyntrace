"""Tests for agentra/monitor/audit_log.py"""
import json
import logging
import os
import time
...
def reset_audit_logger(monkeypatch):
    ...
    import agentra.monitor.audit_log as _mod
    _mod._logger = None
    yield
    _mod._logger = None
...
def test_write_event_creates_json_line(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("AGENTRA_AUDIT_LOG", str(log_path))
...
def test_silently_swallows_errors(monkeypatch):
    ...
    monkeypatch.setenv("AGENTRA_AUDIT_LOG", "/dev/null/impossible/path.log")
    from agentra.monitor.audit_log import write_audit_event
    # Should not raise
...
def test_log_path_from_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("AGENTRA_AUDIT_LOG", str(custom_path))
    from agentra.monitor.audit_log import write_audit_event
