"""agentra alert manager — webhook and threshold-based alerting."""
...
class AlertManager:
    """
    Webhook-based alert manager for agentra scans.
    ...
    def _format_slack(self, event: str, data: dict, severity: str) -> dict:
        ...
        return {
            "attachments": [{
                "color": color,
                "title": f"[agentra] {severity.upper()} — {metric} alert",
                "text": (
    ...
    def _send(self, url: str, payload: dict) -> None:
        ...
        req = urllib.request.Request(
            url,
            data=body,
            headers={"Content-Type": "application/json", "User-Agent": "agentra/0.4.0"},
            method="POST",
        )
