...
def watch(
    ...
    print(f"[agentra] Starting monitoring daemon for {fn_name!r}")
...
def _handle_regression(
    ...
    msg = f"[agentra] REGRESSION in {fn_name!r}: {baseline_rate:.1%} → {current_rate:.1%} (delta: {delta:+.1%})"
    print(f"  ⚠️  {msg}")
...
def _send_webhook(url: str, msg: str, report) -> None:
    ...
    try:
        ...
        print(f"[agentra] Webhook failed: {e}")
