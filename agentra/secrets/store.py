"""Local encrypted secrets store at ~/.agentra/secrets.json.

- Without ``cryptography`` package: plaintext JSON, chmod 600 (warns).
- With ``cryptography`` package + ``AGENTRA_SECRETS_KEY`` env var set:
  AES-256 Fernet encryption (pip install agentra[secure]).
"""
...
_DEFAULT_SECRETS_FILE = Path.home() / ".agentra" / "secrets.json"


def _get_fernet_key() -> bytes | None:
    """Derive a Fernet key from AGENTRA_SECRETS_KEY, or return None."""
    raw = os.getenv("AGENTRA_SECRETS_KEY")
    if not raw:
        return None
    import base64
    import hashlib
...
def load_secrets(path: Path | None = None) -> dict[str, str]:
    ...
    if key:
        try:
            ...
            warnings.warn(
                "[agentra] AGENTRA_SECRETS_KEY set but cryptography not installed. "
                "Run: pip install agentra[secure]"
            )
            return {}
        except Exception:
            warnings.warn(
                "[agentra] Failed to decrypt secrets file — wrong AGENTRA_SECRETS_KEY?"
            )
            return {}
    else:
        try:
            ...
            return {}
        warnings.warn(
            "[agentra] Secrets file loaded without encryption. "
            "Set AGENTRA_SECRETS_KEY and run 'pip install agentra[secure]' to encrypt."
        )
...
def save_secrets(secrets: dict[str, str], path: Path | None = None) -> None:
    ...
    Encrypts with Fernet AES-256 when AGENTRA_SECRETS_KEY is set and
    ...
    if key:
        try:
            ...
            warnings.warn(
                "[agentra] cryptography not installed — saving secrets as plaintext. "
                "Run: pip install agentra[secure]"
            )
