"""HTTP Basic Auth, in-memory rate limiting, and RBAC for the agentra dashboard.

Configuration via environment variables:
  AGENTRA_USERNAME        — dashboard username (enables Basic Auth when set with PASSWORD)
  AGENTRA_PASSWORD        — dashboard password
  AGENTRA_ADMIN_USERS     — comma-separated usernames with admin access
  AGENTRA_READONLY_USERS  — comma-separated usernames restricted to read-only access
  AGENTRA_SECRET_KEY      — signing key for OAuth session cookies
"""
...
def _creds() -> tuple[str, str] | None:
    ...
    u = os.getenv("AGENTRA_USERNAME")
    p = os.getenv("AGENTRA_PASSWORD")
...
def _get_user_role(username: str) -> Role:
    ...
    1. If AGENTRA_READONLY_USERS is set and username is in it → viewer
    2. If AGENTRA_ADMIN_USERS is set and username is NOT in it → viewer
    3. Otherwise → admin
    """
    admins = {u.strip() for u in os.getenv("AGENTRA_ADMIN_USERS", "").split(",") if u.strip()}
    viewers = {u.strip() for u in os.getenv("AGENTRA_READONLY_USERS", "").split(",") if u.strip()}
...
def require_auth(request: object) -> str | None:
    ...
    if scheme.lower() != "basic":
        raise HTTPException(
            401,
            "Unauthorized",
            headers={"WWW-Authenticate": 'Basic realm="agentra"'},
        )
...
def _verify_session_cookie(request: object) -> str | None:
    """Verify the HMAC-signed ``agentra_session`` cookie.
    ...
    cookie = cookies.get("agentra_session")
    if not cookie:
        return None

    secret = os.getenv("AGENTRA_SECRET_KEY", "")
    if not secret:
        return None
...
def make_session_cookie(username: str) -> str:
    ...
    Uses ``AGENTRA_SECRET_KEY`` from env.  Falls back to a random key per
    process if the env var is not set (sessions won't survive restarts).
    """
    import hmac
    import hashlib

    secret = os.getenv("AGENTRA_SECRET_KEY") or _process_secret()
