...
Configuration via environment variables:
  AGENTRA_OAUTH_PROVIDER      — "github" or "google"
  AGENTRA_OAUTH_CLIENT_ID     — OAuth app client ID
  AGENTRA_OAUTH_CLIENT_SECRET — OAuth app client secret
  AGENTRA_OAUTH_REDIRECT_URI  — callback URL (default: http://localhost:7234/auth/callback)
"""
...
def _provider_cfg() -> tuple[str, dict, str, str, str] | None:
    ...
    provider = os.getenv("AGENTRA_OAUTH_PROVIDER", "").lower()
    client_id = os.getenv("AGENTRA_OAUTH_CLIENT_ID", "")
    client_secret = os.getenv("AGENTRA_OAUTH_CLIENT_SECRET", "")
    redirect_uri = os.getenv(
        "AGENTRA_OAUTH_REDIRECT_URI", "http://localhost:7234/auth/callback"
    )
