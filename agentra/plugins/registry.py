"""Community plugin registry — install/list/uninstall agentra plugins."""
...
_REGISTRY_URL = "https://raw.githubusercontent.com/pinexai/agentra-plugins/main/plugins.json"

# Bundled known plugins (fallback if registry unreachable)
_KNOWN_PLUGINS = [
    {
        "name": "advanced-jailbreak",
        "pypi": "agentra-plugin-advanced-jailbreak",
    ...
    {
        "name": "medical-safety",
        "pypi": "agentra-plugin-medical-safety",
    ...
    },
    {
        ...
        "pypi": "agentra-plugin-legal-compliance",
    ...
    },
    {
        ...
        "pypi": "agentra-plugin-multilingual-jailbreak",
...
def install(plugin_name: str, source: str = "agentra-hub") -> None:
    """
    Install a community plugin.

    Usage:
        agentra.plugins.install("advanced-jailbreak")
    """
    ...
    if pypi_name is None:
        ...
        pypi_name = f"agentra-plugin-{plugin_name}"

    print(f"[agentra] Installing plugin '{plugin_name}' ({pypi_name})...")
    result = subprocess.run(
        [sys.executable, "-m", "pip", "install", pypi_name],
        capture_output=False,
    )
    if result.returncode == 0:
        print(f"[agentra] Plugin '{plugin_name}' installed successfully.")
        _register_installed(plugin_name, pypi_name)
    else:
        print(f"[agentra] Failed to install '{plugin_name}'.")
...
def uninstall(plugin_name: str) -> None:
    ...
    print(f"[agentra] Uninstalling plugin '{plugin_name}'...")
    result = subprocess.run(
        [sys.executable, "-m", "pip", "uninstall", "-y", pypi_name],
        capture_output=False,
    )
    if result.returncode == 0:
        print(f"[agentra] Plugin '{plugin_name}' uninstalled.")
    else:
        print(f"[agentra] Failed to uninstall '{plugin_name}'.")


def _register_installed(name: str, pypi: str) -> None:
    """Track installed plugins in agentra DB."""
    try:
        ...
        if hasattr(cls, "name") and hasattr(cls, "generate"):
            ...
            print(f"[agentra] Registered plugin '{cls.name}' in PLUGIN_REGISTRY")
    except Exception:
        pass
