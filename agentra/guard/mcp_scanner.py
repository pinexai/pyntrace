...
class MCPScanReport:
    ...
    def summary(self) -> None:
        print(f"\n{'='*60}")
        print(f"[agentra] MCP Security Report — {self.endpoint}")
        print(f"{'='*60}")
    ...
    def to_sarif(self) -> dict:
        ...
        for r in self.results:
            ...
            sarif_results.append({
                "ruleId": f"agentra/mcp/{r.test_name}",
    ...
    def save_sarif(self, path: str) -> None:
        ...
        print(f"[agentra] MCP SARIF report saved to {path}")
    ...
    def to_junit(self) -> str:
        ...
        lines = [
            ...
            f'<testsuite name="agentra-mcp" tests="{self.total_tests}" failures="{failures}" errors="0">',
        ]
        for test_name, findings in sorted(groups.items()):
            n_vuln = sum(1 for f in findings if f.vulnerable)
            duration = sum(f.duration_ms for f in findings) / 1000
            lines.append(f'  <testcase name="{test_name}" classname="agentra.mcp" time="{duration:.3f}">')
            if n_vuln > 0:
    ...
    def save_junit(self, path: str) -> None:
        ...
        print(f"[agentra] MCP JUnit report saved to {path}")
    ...
    def _persist(self) -> None:
        try:
            ...
            print(f"[agentra] Warning: could not persist MCP scan report: {e}")
...
def scan_mcp(
    ...
    print(f"\n[agentra] MCP Security Scan — {endpoint}")
