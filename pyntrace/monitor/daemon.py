...
def watch(
    ...
    print(f"[pyntrace] Starting monitoring daemon for {fn_name!r}")
...
def _handle_regression(
    ...
    msg = f"[pyntrace] REGRESSION in {fn_name!r}: {baseline_rate:.1%} → {current_rate:.1%} (delta: {delta:+.1%})"
    print(f"  ⚠️  {msg}")
