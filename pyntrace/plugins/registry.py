"""Community plugin registry — install/list/uninstall pyntrace plugins."""
...
_REGISTRY_URL = "https://raw.githubusercontent.com/pinexai/pyntrace-plugins/main/plugins.json"

# Bundled known plugins (fallback if registry unreachable)
_KNOWN_PLUGINS = [
    {
        "name": "advanced-jailbreak",
        "pypi": "pyntrace-plugin-advanced-jailbreak",
        "description": "Extended jailbreak attack templates with 50+ vectors",
        "author": "pyntrace-team",
        "type": "attack",
    },
    {
        "name": "medical-safety",
        "pypi": "pyntrace-plugin-medical-safety",
    ...
    },
    {
        ...
        "pypi": "pyntrace-plugin-legal-compliance",
    ...
    },
    {
        ...
        "pypi": "pyntrace-plugin-multilingual-jailbreak",
...
def install(plugin_name: str, source: str = "pyntrace-hub") -> None:
    """
    Install a community plugin.

    Usage:
        pyntrace.plugins.install("advanced-jailbreak")
    """
    ...
    if pypi_name is None:
        ...
        pypi_name = f"pyntrace-plugin-{plugin_name}"

    print(f"[pyntrace] Installing plugin '{plugin_name}' ({pypi_name})...")
    result = subprocess.run(
        [sys.executable, "-m", "pip", "install", pypi_name],
        capture_output=False,
    )
    if result.returncode == 0:
        print(f"[pyntrace] Plugin '{plugin_name}' installed successfully.")
        _register_installed(plugin_name, pypi_name)
    else:
        print(f"[pyntrace] Failed to install '{plugin_name}'.")
...
def uninstall(plugin_name: str) -> None:
    ...
    print(f"[pyntrace] Uninstalling plugin '{plugin_name}'...")
    result = subprocess.run(
        [sys.executable, "-m", "pip", "uninstall", "-y", pypi_name],
        capture_output=False,
    )
    if result.returncode == 0:
        print(f"[pyntrace] Plugin '{plugin_name}' uninstalled.")
    else:
        print(f"[pyntrace] Failed to uninstall '{plugin_name}'.")


def _register_installed(name: str, pypi: str) -> None:
    """Track installed plugins in pyntrace DB."""
    try:
        ...
        if hasattr(cls, "name") and hasattr(cls, "generate"):
            ...
            print(f"[pyntrace] Registered plugin '{cls.name}' in PLUGIN_REGISTRY")
    except Exception:
        pass
